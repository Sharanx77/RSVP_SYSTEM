      ...
      {!qrCode ? (
        ...
        <div className="flex flex-col items-center justify-center space-y-8 animate-in fade-in zoom-in duration-500 h-full">
          ...
          <div className="flex flex-col gap-4 w-full px-4">
            <button 
              onClick={downloadQR}
              className="w-full bg-emerald-500 text-white font-bold py-3 rounded-xl hover:bg-emerald-400 transition-all active:scale-95"
            >
              ⬇️ Save Ticket to Device
            </button>

            <button 
              onClick={() => { setQrCode(null); setIsBooking(false); setStatus({type: "idle", message: ""}); }} 
              className="text-neutral-500 text-xs font-bold tracking-widest uppercase hover:text-red-400 transition-colors"
            >
              Close Ticket
            </button>
          </div>
